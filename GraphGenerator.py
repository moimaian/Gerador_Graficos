...
ICON_FILENAME = "GraphGenerator.png"
...
def _trendline_xy(ax, xs, ys, trend_mode: str, *, color="#5F5F5F", label=None, linewidth=2):
    """
    Aplica trendline em uma série (xs, ys).
    - Linhas de tendência sempre pontilhadas (:) e com cor controlada por 'color'
...
def plot_bar(
    ...
    if trend_mode != "None":
        ...
        for i, (pos, y, _bars, series_c) in enumerate(bars_by_series):
            ...
            _trendline_xy(ax, pos, y, trend_mode, color=tcolor, label=f"Trend ({series_name})", linewidth=2)
