...
class GraphGeneratorUI(QWidget):
    ...
    def _update_chart_type_options(self, df: pd.DataFrame):
        # opções base (1D)
        base_1d = [
            "Bar", "Line", "Scatter", "Area (Stacked)", "Area (Overlapped)", "Box Plot", "Heatmap",
        ...
        if len(x_sel) != 1:
            opts = ["Bar", "Box Plot", "Violin", "Histogram", "Density (KDE)", "Scatter"]
        else:
            x_col = x_sel[0]
            ...
            if not x_is_num:
                ...
                if len(y_sel) == 0:
                    ...
                    opts = ["Bar", "Box Plot", "Violin", "Histogram", "Density (KDE)"]
            else:
