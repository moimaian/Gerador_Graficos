...
def _is_categorical_series(s: pd.Series) -> bool:
    """Heurística: é categórica se menos de 50% vira número."""
    sn = pd.to_numeric(s, errors="coerce")
    return sn.notna().mean() < 0.5
...
def plot_area_stacked(df: pd.DataFrame, dpi: int, trendline: bool, title: str, xlabel: str = "", ylabel: str = "", label_mark: bool = False,
              color_mode: str = "Palette (multi-color)",
              palette_name: str = "Dark2",
              single_color: str = "#1f77b4"):
...
def plot_violin(df: pd.DataFrame, dpi: int, trendline: bool, label_mark: bool, title: str,
    ...
    # Caso comum em 1D: X categórico e 1 coluna Y -> violino por classe/categoria
    if len(ydf.columns) == 1 and (not np.issubdtype(np.asarray(x).dtype, np.number)):
        ycol = ydf.columns[0]
        tmp = pd.DataFrame({"X": x, "Y": pd.to_numeric(ydf[ycol], errors="coerce")}).dropna()

        groups = [g["Y"].to_numpy(dtype=float) for _, g in tmp.groupby("X", sort=False)]
        labels = [str(k) for k, _ in tmp.groupby("X", sort=False)]
        ...
        if color_mode.startswith("Single"):
            for pc in parts["bodies"]:
                pc.set_facecolor(single_color)
                pc.set_alpha(0.6)
        else:
            # uma cor por violino (categoria)
        ...
        # Se houver múltiplas colunas Y: violino por coluna (distribuição por série)
        data = [ydf[c].dropna().to_numpy(dtype=float) for c in ydf.columns]
        ...
        ax.set_xticks(np.arange(1, len(ydf.columns) + 1))
        ax.set_xticklabels([str(c) for c in ydf.columns], rotation=0, ha="center")
...
def plot_histogram(df: pd.DataFrame, dpi: int, trendline: bool, label_mark: bool, title: str,
                   xlabel: str = "", ylabel: str = "",
                   color_mode: str = "Palette (multi-color)",
                   palette_name: str = "Dark2",
                   single_color: str = "#1f77b4"):
...
def plot_density(df: pd.DataFrame, dpi: int, trendline: bool, label_mark: bool, title: str,
                 xlabel: str = "", ylabel: str = "",
                 color_mode: str = "Palette (multi-color)",
                 palette_name: str = "Dark2",
                 single_color: str = "#1f77b4"):
...
def plot_violin_2d_category(df: pd.DataFrame, group_col: str, y_col: str,
    ...
    ax.set_xlabel(xlabel or group_col)
    ax.set_ylabel(ylabel or y_col)
    ax.set_title(title, fontsize=14, fontweight="bold") if title else ax.set_title("Violin Plot", fontsize=14)
    ax.spines[["top", "right"]].set_visible(False)
    plt.tight_layout()
    return fig
...
class GraphGeneratorUI(QWidget):
    def __init__(self):
        ...
        # --- Data mapping (Dimensions + X/Y/Z columns) ---
        map_box = QGroupBox("Data mapping")
        map_grid = QGridLayout(map_box)

        # Dimensions
        map_grid.addWidget(QLabel("Dimensions:"), 0, 0)

        self.ck_1d = QCheckBox("1D")
        self.ck_2d = QCheckBox("2D")
        self.ck_3d = QCheckBox("3D")

        map_grid.addWidget(self.ck_1d, 0, 1)
        map_grid.addWidget(self.ck_2d, 0, 2)
        map_grid.addWidget(self.ck_3d, 0, 3)

        # Default (keep current behavior): 1D
        self.ck_1d.setChecked(True)

        # Column selectors
        map_grid.addWidget(QLabel("X column (single):"), 1, 0)
        self.lw_x = QListWidget()
        self.lw_x.setSelectionMode(QAbstractItemView.SingleSelection)
        map_grid.addWidget(self.lw_x, 1, 1, 1, 3)

        map_grid.addWidget(QLabel("Y columns (numeric, multi):"), 2, 0)
        self.lw_y = QListWidget()
        self.lw_y.setSelectionMode(QAbstractItemView.ExtendedSelection)
        map_grid.addWidget(self.lw_y, 2, 1, 1, 3)

        map_grid.addWidget(QLabel("Z columns (numeric, multi):"), 3, 0)
        self.lw_z = QListWidget()
        self.lw_z.setSelectionMode(QAbstractItemView.ExtendedSelection)
        map_grid.addWidget(self.lw_z, 3, 1, 1, 3)
        ...
        root.insertWidget(1, map_box)  # put mapping box right after the first group
    ...
    def _update_chart_type_options(self, df: pd.DataFrame):
        # opções base (1D)
        base_1d = [
            "Bar", "Line", "Scatter", "Area (Stacked)", "Box Plot", "Heatmap",
            "Violin Plot", "Histogram", "Density (KDE)"
        ]

        # se não estiver em 2D, mantém tudo
        if not self.ck_2d.isChecked():
        ...
        # --- 2D rules ---
        x_sel = self._get_selected_columns(self.lw_x)
        y_sel = self._get_selected_columns(self.lw_y)  # em 2D deve ter 0 ou 1

        # sem seleção ainda → mostra subset seguro
        if len(x_sel) != 1:
            opts = ["Bar", "Box Plot", "Violin Plot", "Histogram", "Density (KDE)", "Scatter"]
        else:
            x_col = x_sel[0]
            x_is_num = self._is_column_numeric(df, x_col)

            if not x_is_num:
                # X categórico
                if len(y_sel) == 0:
                    # sem Y: só faz sentido Bar (frequência)
                    opts = ["Bar"]
                else:
                    # com Y: distribuição por categoria
                    opts = ["Bar", "Box Plot", "Violin Plot", "Histogram", "Density (KDE)"]
            else:
                # X numérico: para 2D, Scatter é o mais coerente (X vs Y)
                opts = ["Scatter"]

        self.cb_plot.blockSignals(True)
        cur = self.cb_plot.currentText()
        self.cb_plot.clear()
        self.cb_plot.addItems(opts)
        if cur in opts:
            self.cb_plot.setCurrentText(cur)
        self.cb_plot.blockSignals(False)
    ...
    def _on_dim_changed(self):
        ...
        # Em 1D: Y multi; Em 2D: Y single; (3D você define depois)
        if is_1d:
            self.lw_y.setSelectionMode(QAbstractItemView.ExtendedSelection)
        elif is_2d:
            ...
            # 3D (placeholder): por enquanto single também
            self.lw_y.clearSelection()
            self.lw_y.setSelectionMode(QAbstractItemView.SingleSelection)
    ...
    def _populate_column_lists(self, df: pd.DataFrame):
        ...
        self.lw_x.addItems(cols)
        self.lw_y.addItems(cols)
        self.lw_z.addItems(cols)

        # Detect numeric columns (robust to comma decimal)
        numeric_cols = []
        for c in df.columns:
            s = _series_to_numeric(df[c])
            # consider numeric if at least 50% convertible to numeric
            if s.notna().mean() >= 0.5:
        ...
        # Keep default safe selection as 1D
    ...
    def generate_chart(self):
        ...
        if self.ck_2d.isChecked():
            sel_x = self._get_selected_columns(self.lw_x)
            sel_y = self._get_selected_columns(self.lw_y)  # 0 ou 1
            if len(sel_x) != 1:
                _warn(self, "Warning", "In 2D mode you must select exactly ONE X column.")
                return

            x_col = sel_x[0]
            y_col = sel_y[0] if len(sel_y) == 1 else None

            x_is_num = self._is_column_numeric(df, x_col)
            plot_type = self.cb_plot.currentText().strip()

            # Case 1: X is categorical => series are the categories of X
            if not x_is_num:
                group_col = x_col

                # If Y not selected -> frequency (only Bar should be available)
                if y_col is None:
                    fig = plot_bar_2d_category(
                        df=df,
                        group_col=group_col,
                        y_col=None,
                        title=title,
                ...
                # Y selected: must be numeric
                if not self._is_column_numeric(df, y_col):
                    _warn(self, "Warning", "In 2D mode with categorical X, Y must be numeric.")
                    return

                if plot_type == "Bar":
                    fig = plot_bar_2d_category(
                        df=df, group_col=group_col, y_col=y_col,
                        title=title, xlabel=xlabel, ylabel=ylabel,
                        color_mode=color_mode, palette_name=palette_name,
                        single_color=single_color, label_mark=label_mark
                    )
                elif plot_type == "Box Plot":
                    fig = plot_box_2d_category(
                        df=df, group_col=group_col, y_col=y_col,
                        title=title, xlabel=xlabel, ylabel=ylabel,
                        color_mode=color_mode, palette_name=palette_name,
                        single_color=single_color, label_mark=label_mark
                    ...
                    _warn(self, "Warning", "This chart type is not enabled for categorical X in 2D mode.")
                    return
            ...
            # Case 2: X is numeric and Y is numeric -> use Class/Group as series if available
            if y_col is None:
                _warn(self, "Warning", "In 2D mode with numeric X you must select ONE numeric Y column.")
                return

            if not self._is_column_numeric(df, y_col):
                _warn(self, "Warning", "Selected Y column must be numeric.")
                return

            group_col = _find_group_column(df)
            if group_col is None:
                _warn(self, "Warning", "No group column found (Class/Classes/Group/Groups). Add one or select a categorical X instead.")
                return

            # Scatter only (as per your rule)
